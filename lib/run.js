...
module.exports = async opts => {
  ...
  config.output = {
    ...
    filename: `${opts.basename ? (opts.basename || '') + '/' : ''}bundle.js`,
    publicPath: '/'
  };

  config.resolve.modules.unshift(opts.dirname, path.join(opts.dirname, 'node_modules'));
  ...
  if (opts.analyze) {
    ...
    const analyzerPort = typeof opts.analyze === 'string' ? opts.analyze : 8888;
    config.plugins.push(
  ...
  const serveOpts = {
    ...
    port: opts.port,
    host: opts.host,
    hot: process.env.NODE_ENV === 'production' ? false : {logLevel: 'error'},
  ...
  return new Promise((resolve, reject) => {
    serve(serveOpts)
      .then(server => {
        server.compiler.hooks.done.tap({name: 'reunify'}, stats => {
