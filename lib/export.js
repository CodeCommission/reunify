...
const getApp = opts => {
  ...
  return new Promise((resolve, reject) => {
    compiler.run((err, stats) => {
      ...
      const App = require(path.resolve(opts.tempdir, './App.js'));
      resolve(App);
    });
  });
};
...
const getCSSLibrary = opts => {
  ...
  const deps = Object.assign({}, opts.pkg.devDependencies, opts.pkg.dependencies);
...
const renderHTML = ({opts, routes, App, props, path}) => {
  ...
  switch (cssLibrary) {
    case 'styled-components':
      ...
      html = render(sheet.collectStyles(app));
...
const getRoutes = async App => {
    ...
    routes.map(async route => {
      ...
      await Promise.all(
        route.props.routes.map(async subroute => {
          const pageProps =
            typeof route.Component.getInitialProps === 'function'
  ...
  const staticRoutes = [...routes.filter(route => !route.props.routes), ...dynamicRoutes.filter(route => !!route)];
  return {routes, staticRoutes};
};

module.exports = async opts => {
  // mutation
  baseConfig.resolve.modules.unshift(path.join(opts.dirname, 'node_modules'), opts.dirname);
