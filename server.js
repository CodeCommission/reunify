...
app.all('*', (req, res) => {
  ...
  ReactRouter.match({routes, location: req.url}, (err, redirectLocation, renderProps) => {
    ...
    if (renderProps) {
      ...
      const context = {};
      const promises = renderProps.components.filter(component => component.getJSON || component.getInitialProps).map(
        component =>
          component.getJSON
            ? component.getJSON(req, res, context).then(initialPropsData => ({
                json: true,
                componentInitialPropsData: initialPropsData
              }))
            : component.getInitialProps(req, res, context).then(initialPropsData => ({
                name: component.name,
                componentInitialPropsData: Object.assign({}, initialPropsData, context)
              }))
      );
    ...
    start: ({port, host}) =>
      build().then(
        () =>
          new Promise(
            resolve =>
              (server = app.listen(port, host, () => resolve({url: `http://${host}:${server.address().port}`})))
          )
      ),
