...
if (!IS_PROD)
  app.use(
    require("webpack-hot-middleware")(compiler, { noInfo: true, quiet: true })
  );
...
app.get("/sw.js", (req, res) => {
  ...
  fse
    ...
    .then(text =>
      res.send(text.toString().replace("__TIMESTAMP__", TIMESTAMP))
    );
});
...
app.get("/manifest.json", (req, res) => {
  ...
  fse
    ...
    .then(
      exists =>
        exists ? fs.createReadStream(manifestFilePath).pipe(res) : res.send({})
    );
});

app.get("/assets-manifest.json", (req, res) => {
  res.setHeader("Content-Type", "application/json");
  const assetsManifestFilePath = path.join(
    process.cwd(),
    ...
    .then(
      exists =>
        exists
          ...
          : fs.readdir(path.join(process.cwd(), "/static"), (error, data) =>
              res.send(data)
            )
    );
});
  ...
  ReactRouter.match(
    ...
    (err, redirectLocation, renderProps) => {
      ...
      if (renderProps) {
        let statusCode = 200;
        req.params =
          renderProps.router && renderProps.router.params
            ? renderProps.router.params
            : req.params;
        const context = {};
        const promises = renderProps.components
          ...
          .map(
            component =>
              component.getJSON
                ? component
                    .getJSON(req, res, context)
                    .then(initialPropsData => ({
                      json: true,
                      componentInitialPropsData: initialPropsData
                    }))
                : component
                    .getInitialProps(req, res, context)
                    .then(initialPropsData => ({
                      name: component.name,
                      componentInitialPropsData: Object.assign(
                        {},
                        initialPropsData,
                        context
                      )
        ...
        Promise.all(promises)
          .then(data => {
            ...
            if (jsonDataIndex !== -1)
              return res.send(data[jsonDataIndex].componentInitialPropsData);

            if (!IS_BOWSER && !IS_PROD)
              Object.keys(require.cache).forEach(x => delete require.cache[x]);
            const html = ReactDOM.renderToStaticMarkup(
              <CookiesProvider
                cookies={new Cookies(req.universalCookies.cookies)}
              >
                <ReactRouter.RouterContext
                  {...renderProps}
                  createElement={(Component, props) => {
                    statusCode = props.route.is404 ? 404 : 200;
                    const componentInitialPropsData = (data.find(
          ...
          .catch(error => {
            error =
              error ||
              new Error(
                "Reunify - unknown server (e.g. getInitialProps) error."
              );
...
function renderError(sheet, error) {
  ...
  return ReactDOM.renderToString(
    sheet.collectStyles(<ErrorComponent error={error} />)
  );
}
          ...
          new Promise(
            resolve =>
              (server = app.listen(port, host, () =>
                resolve({ url: `http://${host}:${server.address().port}` })
              ))
          )
      ),
